 ...
 */
import{html,nothing,render}from"/lib/lit/lit.all.mjs";class TemplateElement extends HTMLElement{slotChildren=!1;constructor(){super(),[...this.attributes].forEach(t=>this[t.nodeName]=t.nodeValue),this.class=this.hasAttribute("class")?" "+this.class:"",this.tooltip=this.hasAttribute("tooltip")?this.tooltip||this.id+"-tooltip":void 0,this.display=this.hasAttribute("display")?this.display:void 0}connectedCallback(){if(render(this.template(),this),this.slotChildren){const t=[...this.children];for(let i=0;i<t.length-1;i++){t[i].dataset.slot=this.id}}this.replaceWith(...this.children)}template(){return html``}}class MDButton extends TemplateElement{template(){return this.href?html`
      <a class="mdc-button mdc-button--${this.type||"raised"}${this.class}" id="${this.id}-button" data-tooltip="${this.tooltip??nothing}" data-display="${this.display??nothing}" href="${this.href}" target="${this.target?nothing:"_blank"}">
    ...
    `:html`
      <button class="mdc-button mdc-button--${this.type||"raised"}${this.class}" id="${this.id}-button" data-tooltip="${this.tooltip??nothing}" data-display="${this.display??nothing}">
        <svg data-lit-remove="${this.hasAttribute("icon")?nothing:""}">
          <use href="/lib/${this.library??"fontawesome"}/${this.icon??"solid.svg#asterisk"}"></use>
        </svg>
        <span class="mdc-button__ripple"></span>
    ...
    `}}class MDCheckbox extends TemplateElement{template(){const t=this.hasAttribute("table-header"),i=this.hasAttribute("table-content");return html`
      <div id="${this.id?this.id+"-form-field":nothing}"
           data-slot="${this.dataset.slot??nothing}"
           class="${t||i?nothing:"mdc-form-field"+this.class}"
           data-tooltip="${this.tooltip??nothing}"
           data-display="${this.display??nothing}">
        <div class="mdc-checkbox${t?" mdc-data-table__header-row-checkbox":""}${i?" mdc-data-table__row-checkbox":""}" id="${this.id?this.id+"-checkbox":nothing}">
          <input type="checkbox" class="mdc-checkbox__native-control" id="${this.id?this.id+"-input":nothing}" aria-label="${t?"Checkbox for header row selection":nothing}" name="${t||i?"checkbox":nothing}" value="${this.value??nothing}" data-value="${this.dataset.value??nothing}">
          <div class="mdc-checkbox__background">
            <svg class="mdc-checkbox__checkmark" viewBox="0 0 24 24">
              <path class="mdc-checkbox__checkmark-path" fill="none" d="M1.73,12.91 8.1,19.28 22.79,4.59">
            </svg>
            <div class="${t||i?"mdc-checkbox__mixedmark":""}"></div>
          </div>
    ...
    `}}class MDChip extends TemplateElement{template(){return html`
      <div class="mdc-chip${this.class}" role="row" id="${this.id}-chip" data-slot="${this.dataset.slot}" data-value="${this.value}" data-tooltip="${this.tooltip??nothing}" data-display="${this.display??nothing}">
    ...
    `}}class MDChipSet extends TemplateElement{slotChildren=!0;template(){return html`
      <div class="chip-set-container${this.class}" id="${this.id}-chip-set-container" data-display="${this.display??nothing}">
    ...
    `}}class MDDataTable extends TemplateElement{template(){return html`
      <div class="mdc-data-table${this.class}" id="${this.id}-data-table" data-display="${this.display??nothing}">
    ...
    `}}class MDDialog extends TemplateElement{template(){return html`
      <div class="mdc-dialog${this.class}" role="alertdialog" aria-modal="true" aria-labelledby="${this.id}" aria-describedby="${this.id}-dialog-content" data-no-focus="${this["no-focus"]??nothing}" data-display="${this.display??nothing}" id="${this.id}">
    ...
    `}}class MDDialogButton extends TemplateElement{template(){return this.hasAttribute("href")?html`
      <a type="button" class="mdc-button mdc-dialog__button${this.class}" id="${this.id?this.id:nothing}" href="${this.href}" target="_blank" data-display="${this.display??nothing}">
        <span class="mdc-button__label" data-i18n="${this.i18n||this.id}"></span>
      </a>
    `:html`
      <button type="button" class="mdc-button mdc-dialog__button${this.class}" data-mdc-dialog-action="${"nothing"===this.action?nothing:this.hasAttribute("action")?this.action:"close"}" id="${this.id?this.id:nothing}" data-display="${this.display??nothing}">
        <span class="mdc-button__label" data-i18n="${this.i18n||this.id}"></span>
      </button>
    `}}class MDDrawer extends TemplateElement{template(){return html`
      <div class="drawer-frame-root${this.class}" data-display="${this.display??nothing}">
    ...
    `}}class MDFab extends TemplateElement{template(){return html`
      <div class="fab-div" data-tooltip="${this.id}-fab-tooltip" data-tooltip-placement="${this.hasAttribute("tooltip-placement")?this.getAttribute("tooltip-placement"):nothing}" id="${this.id}-fab-div" data-display="${this.display??nothing}">
    ...
    `}}class MDLinearProgress extends TemplateElement{template(){return html`
      <div role="progressbar" class="mdc-linear-progress${this.class}" aria-label="Progress Bar" aria-valuemin="0" aria-valuemax="1" aria-valuenow="0" id="${this.id}-linear-progress" data-display="${this.display??nothing}">
    ...
    <div class="mdc-list-group${this.class}" id="${this.id}" data-display="${this.display??nothing}">
    ...
    `}}class MDList extends TemplateElement{slotChildren=!0;template(){return html`
      <ul class="mdc-list${this.class}" id="${this.id}" data-slot="${this.dataset.slot}" data-display="${this.display??nothing}">
        <slot id="${this.id}-slot"></slot>
      </ul>
    `}}class MDListItem extends TemplateElement{template(){const t=this.getAttribute("icon")?.startsWith("/");return html`
      <li id="${this.id}-list-item" class="mdc-list-item${this.class}" data-value="${this.value??nothing}" data-slot="${this.dataset.slot}" data-display="${this.display??nothing}">
    ...
    `}}class MDRadio extends TemplateElement{constructor(){super(),this.id=this.id||this.name+"-"+this.value,this.tooltip=this.tooltip?this.id+"-tooltip":void 0}template(){return html`
      <div id="${this.id?this.id+"-form-field":nothing}" class="mdc-form-field${this.class}" data-tooltip="${this.tooltip??nothing}" data-display="${this.display??nothing}">
    ...
    `}}class MDSelect extends TemplateElement{slotChildren=!0;template(){const t=this.hasAttribute("no-label");return html`
      <div class="mdc-select mdc-select--outlined mdc-select--no-label${this.class}" id="${this.id}-select" data-display="${this.display??nothing}">
    ...
    `}}class MDSelectItem extends TemplateElement{template(){return html`
      <li class="mdc-list-item${this.class}" role="option" data-value="${this.value??nothing}" data-slot="${this.dataset.slot??nothing}" data-display="${this.display??nothing}">
        <span class="mdc-list-item__ripple"></span>
        <span class="mdc-list-item__text" id="${this.id}-option" data-i18n="${this.textContent?nothing:""}">${this.textContent??nothing}</span>
      </li>
    `}}class MDSlider extends TemplateElement{template(){return html`
      <div class="mdc-slider mdc-slider--discrete" tabindex="0" id="${this.id}-slider" role="slider" aria-valuemin="${this.min??0}" aria-valuemax="${this.max??100}" aria-valuenow="${this.now??0}" aria-label="Select Value" data-display="${this.display??nothing}">
    ...
    `}}class MDSnackbar extends TemplateElement{template(){const t=this.hasAttribute("undo");return html`
      <div class="mdc-snackbar${this.class}" id="${this.id}-snackbar" data-display="${this.display??nothing}">
    ...
    `}}class MDSwitch extends TemplateElement{template(){const t=this.hasAttribute("left"),i=this.hasAttribute("icon");return t?html`
      <div class="mdc-switch-container${this.class} left" id="${this.id}-switch-container" data-tooltip="${this.tooltip??nothing}" data-display="${this.display??nothing}">
        <div class="mdc-switch" id="${this.id}-switch">
        ...
        <svg class="mdc-switch__icon" id="${this.id}-icon" data-lit-remove="${i?nothing:""}"><use href="/lib/fontawesome/${this.icon??"solid.svg#asterisk"}"></use></svg>
        <label class="mdc-switch__label" for="${this.id}-input" id="${this.id}-label" data-i18n="${this.id}-switch"></label>
      </div>
    `:html`
      <div class="mdc-switch-container${this.class}" id="${this.id}-switch-container" data-tooltip="${this.tooltip??nothing}" data-display="${this.display??nothing}">
        <svg class="mdc-switch__icon" id="${this.id}-icon" data-lit-remove="${i?nothing:""}"><use href="/lib/fontawesome/${this.icon??"solid.svg#asterisk"}"></use></svg>
        <label class="mdc-switch__label" for="${this.id}-input" id="${this.id}-label" data-i18n="${this.id}-switch"></label>
        <div class="mdc-switch" id="${this.id}-switch">
          <div class="mdc-switch__track"></div>
    ...
    `}}class MDTabBar extends TemplateElement{slotChildren=!0;template(){return html`
      <nav class="mdc-tab-bar${this.class}" role="tablist" id="${this.id}" data-display="${this.display??nothing}">
    ...
    `}}class MDTab extends TemplateElement{template(){const t=this.hasAttribute("unstacked")?"":" mdc-tab--stacked",i=this.hasAttribute("active"),s=this.parentElement?.id+"-"+this.tab;return html`
      <button class="mdc-tab${t}${i?" mdc-tab--active":""}${this.class}" role="tab" tabindex="${i?"0":"-1"}" aria-selected="${i?"true":nothing}" data-slot="${this.dataset.slot??nothing}" data-tab="${this.tab}" data-display="${this.display??nothing}">
        <span class="mdc-tab__content">
          <svg class="mdc-tab__icon" aria-hidden="true"><use href="/lib/fontawesome/${this.icon}"></use></svg>
          <span class="mdc-tab__text-label" id="${s}" data-i18n></span>
          <span class="mdc-tab-indicator${i?" mdc-tab-indicator--active":""}">
            <span class="mdc-tab-indicator__content mdc-tab-indicator__content--underline"></span>
          </span>
        </span>
        <span class="mdc-tab__ripple mdc-ripple-upgraded"></span>
      </button>
    `}}class MDTextarea extends TemplateElement{constructor(){super(),this.fullwidth=this.hasAttribute("fullwidth")?" mdc-text-field--fullwidth":"",this.spellcheck=this.hasAttribute("spellcheck")?this.getAttribute("spellcheck"):void 0}template(){return html`
      <div class="mdc-text-field mdc-text-field--textarea${this.fullwidth}${this.class}" id="${this.id}-text-field" data-tooltip="${this.tooltip??nothing}" data-display="${this.display??nothing}">
    ...
    `}}class MDTextField extends TemplateElement{constructor(){super(),this.spellcheck=this.hasAttribute("spellcheck")?this.getAttribute("spellcheck"):void 0}template(){const t=this.hasAttribute("no-label");return html`
      <div class="mdc-text-field mdc-text-field--outlined${this.class}" id="${this.id}-text-field" data-tooltip="${this.tooltip??nothing}" data-display="${this.display??nothing}">
    ...
    `}}class MDTextFieldHelperLine extends TemplateElement{slotChildren=!0;template(){return html`
      <div id="${this.id}-text-field-helper-line" class="mdc-text-field-helper-line${this.class}" data-display="${this.display??nothing}">
    ...
    `}}class MDIcon extends TemplateElement{constructor(){super(),this.href=this.hasAttribute("href")?this.href:"/lib/"+(this.library||"fontawesome")+"/"+this.icon}template(){return html`
      <svg id="${this.id||nothing}" class="${this.class||nothing}" data-value="${this.dataset.value??nothing}" data-tooltip="${this.tooltip??nothing}" data-display="${this.display??nothing}">
    ...
    `}}class OptionsHeader extends TemplateElement{slotChildren=!0;template(){return html`
      <header id="options-header" class="grid-row" data-display="${this.display??nothing}">
        <div>
          <svg id="options-header-icon"><use href="/app/media/symbol.svg#icon"></use></svg>
          <span id="name" data-i18n></span>
          <span class="spxx-badge" data-i18n="spxx-badge"></span>
    ...
    `}}class SixLogo extends TemplateElement{template(){const t=this.hasAttribute("no-link"),i=this.hasAttribute("website");return t?html`
      <div id="${this.id?this.id:nothing}" class="six-logo" data-display="${this.display??nothing}">
        <div class="six-logo-inner">
          <div class="six-logo-mwhby">
            <span>mw</span>
            <md-icon class="six-logo-heart hvr-grow-2x" icon="solid.svg#heart"></md-icon>
            <span>by</span>
          </div>
          <div class="six-logo-6">6</div>
        </div>
      </div>
      `:html`
      <div id="${this.id?this.id:nothing}" class="six-logo" data-display="${this.display??nothing}">
        <a class="six-logo-inner" href="${i?"/":"https://sixcious.com"}" target="${i?"_self":"_blank"}">
          <div class="six-logo-mwhby">
            <span>mw</span>
            <md-icon class="six-logo-heart hvr-grow-2x" icon="solid.svg#heart"></md-icon>
            <span>by</span>
    ...
    `}}class Template{static#t=(()=>{Template.#i(),Template.#s(),Template.#e()})();static#i(){for(const t of["md-data-table","md-data-table-row"]){const i=document.querySelectorAll(t);for(const t of i){let i=1;for(const s of t.children)s.id||(s.id=t.id+"-child-"+i++,s.setAttribute("data-remove-id",""))}}const t=document.querySelectorAll("md-data-table-row");for(const i of t)for(const t of i.children)t.type=i.slot.endsWith("header")?"header":"content"}static#s(){customElements.define("md-button",MDButton),customElements.define("md-checkbox",MDCheckbox),customElements.define("md-chip-set",MDChipSet),customElements.define("md-chip",MDChip),customElements.define("md-fab",MDFab),customElements.define("md-data-table",MDDataTable),customElements.define("md-data-table-row",MDDataTableRow),customElements.define("md-data-table-cell",MDDataTableCell),customElements.define("md-dialog",MDDialog),customElements.define("md-dialog-button",MDDialogButton),customElements.define("md-drawer",MDDrawer),customElements.define("md-linear-progress",MDLinearProgress),customElements.define("md-list-group",MDListGroup),customElements.define("md-list",MDList),customElements.define("md-list-item",MDListItem),customElements.define("md-radio",MDRadio),customElements.define("md-select",MDSelect),customElements.define("md-select-item",MDSelectItem),customElements.define("md-slider",MDSlider),customElements.define("md-snackbar",MDSnackbar),customElements.define("md-switch",MDSwitch),customElements.define("md-tab",MDTab),customElements.define("md-tab-bar",MDTabBar),customElements.define("md-textarea",MDTextarea),customElements.define("md-text-field",MDTextField),customElements.define("md-text-field-helper-line",MDTextFieldHelperLine),customElements.define("md-icon",MDIcon),customElements.define("options-header",OptionsHeader),customElements.define("options-section",OptionsSection),customElements.define("six-logo",SixLogo),customElements.define("support-hearts",SupportHearts)}static#e(){const t=document.querySelectorAll("[data-slot]");for(const i of t){const t=document.getElementById(i.dataset.slot+"-slot");if(t)try{t.appendChild(i)}catch(t){}i.removeAttribute("data-slot")}const i=document.querySelectorAll("slot");for(const t of i)t.replaceWith(...t.children);const s=document.querySelectorAll("[data-slot-remove],[data-lit-remove]");for(const t of s)t.remove();const e=document.querySelectorAll("[data-remove-id]");for(const t of e)t.removeAttribute("id"),t.removeAttribute("data-remove-id");const a=document.querySelectorAll("[data-template]");for(const t of a){const i=document.createElement("template");i.id=t.dataset.template,i.content.appendChild(t),document.body.appendChild(i),t.removeAttribute("id"),t.removeAttribute("data-template")}}}